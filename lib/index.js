...
function compileScenarios(src, lang) {
  var output = {
    code: '',
    steps: {},
    patterns: {},
  ...
  _.each(glob.sync(src + '/**/*.{litcoffee,coffee.md}'), function(file) {
...
function compileFeatures(src, lang) {
  var parser = new Yadda.parsers.FeatureParser(lang);

  return _.object(_.map(new Yadda.FeatureFileSearch(src).list(), function(file) {
...
module.exports = function (options) {
  ...
  var features = compileFeatures(params.features, Yadda.localisation[language] || Yadda.localisation.English);
  ...
  var scenarios = compileScenarios(params.steps, language),
      library_code = library(_.merge({}, scenarios, { yadda: require.resolve('yadda') }));

  fs.outputFileSync(path.resolve(params.dest + '/helpers/_library.js'), coffee.compile(library_code, { bare: true }));

  if (fs.existsSync(params.helpers)) {
    _.each(glob.sync(params.helpers + '/**/*.{js,coffee,coffee.md,litcoffee}'), function(file) {
      var helper_file = path.resolve(params.dest + '/helpers/' + file.replace(params.helpers, '')),
