...
function compileTests(params) {
  ...
  sorted.sort(function(a, b) {
    var x = a.feature.annotations.order || 0,
        y = b.feature.annotations.order || 0;

    return x - y;
  });

  _each(sorted, function(item) {
    var feature = item.feature,
        file = item.file;

    var length = feature.scenarios.length;

    feature.scenarios.forEach(function(scenario, key) {
      key = ('0000000' + key).substr(-length);

      var feature_name = path.basename(file, '.feature') + '/' + key + '_' + normalize(scenario.title),
