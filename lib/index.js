...
function extractSteps(file) {
  ...
  lines.forEach(function(line, offset) {
    ...
    if (line.trim()) {
      testLine = matchLabel(line, lines, offset);
...
function compileScenarios(params) {
  ...
  var steps_dir = path.join(params.src, 'steps');

  _.each(glob.sync(path.join(steps_dir, '**/*.{litcoffee,coffee.md}')), function(file) {
...
function compileTests(params) {
  ...
  var engine = _.template(fs.readFileSync(path.join(__dirname, '_runner.coffee')).toString()),
  ...
  _.each(features, function(feature, file) {
    var feature_name = path.basename(file, '.feature') + '-' + params.suffix,
